...
Feature: Job Search
  ...
  Scenario Outline: End to End usage of search functionality
    ...
    Examples:
    |What       |Where    |JobReference   |Employer                       |Distance   |WorkingPattern|ContractType |StaffGroup           |PayRange           |PayGrade   |
...
@search
  Scenario: End to End usage of search functionality
    ...
    And I click on "search button"
    And I filter the search result for "working pattern" "full-time"
    And I filter the search result for "contract type" "permanent"
    And I filter the search result for "staff group" "Healthcare Scientists"
    And I filter the search result for "pay range" "£0 to £10,000"
    And I filter the search result for "pay grade" "Specialty Doctor"
    And results should contain some job adverts
     #Handled No results found here